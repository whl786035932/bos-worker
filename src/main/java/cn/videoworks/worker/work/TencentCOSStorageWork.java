...
import cn.videoworks.worker.function.WriteTencentCOSStorage;
 ...
 * @see 	 
 */
public class TencentCOSStorageWork {
	
	private static Logger log = LoggerFactory.getLogger(TencentCOSStorageWork.class);
	...
	public static void registWorker(String storageClient){
        ...
        if(StringUtils.isNotBlank(gearmanIp) && -1 != gearmanPort) {
	        ...
	        worker.addFunction(WorkerFunctionConstant.TENCENT_COS_OBJ_INSTORAGE, new WriteTencentCOSStorage());
