...
import cn.videoworks.worker.function.NetEaseFunction;
 ...
 *
 */
public class NetEaseWorker {
	private static Logger log = LoggerFactory.getLogger(NetEaseWorker.class);
	...
	public static void registWorker(){
		...
		if (StringUtils.isNotBlank(gearmanIp) && -1 != gearmanPort) {
			...
			worker.addFunction(WorkerFunctionConstant.NETEASE, new NetEaseFunction());
