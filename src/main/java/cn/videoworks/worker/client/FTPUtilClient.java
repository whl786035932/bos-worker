...
public class FTPUtilClient {
	...
	public static FTPUtilClient getInstance() {
		synchronized (FTPUtilClient.class) {
			if (instance == null) {
		...
		if (null != ftpClient && ftpClient.isConnected()) {
			...
			try {
				...
				try {
					...
					port = Integer.valueOf(PropertiesUtil.getPropertiesUtil().get("ftp.port"));
					logger.info("ftp.port=" + port);
					username = PropertiesUtil.getPropertiesUtil().get("ftp.username");
					logger.info("ftp.username=" + username);
					password = PropertiesUtil.getPropertiesUtil().get("ftp.password");
...
//			outputStreamWriter.close();
	...
	public static void main(String[] args) throws Exception {
		FTPUtilClient client = FTPUtilClient.getInstance();
